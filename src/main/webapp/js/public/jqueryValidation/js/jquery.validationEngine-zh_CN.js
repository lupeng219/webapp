...
/*	正式机
    var  webroot=document.location.href;
	webroot=webroot.substring(webroot.indexOf('//')+2,webroot.length);
	loginroot=webroot.substring(0,webroot.indexOf("/")+1);
	webroot=webroot.substring(webroot.indexOf('/')+1,webroot.length);
	webroot=webroot.substring(0,webroot.indexOf('/'));
	if(webroot==null){
	   rootpath=loginroot;
	}else{
	   rootpath=loginroot+webroot;
	}
*/
	...
	$.validationEngineLanguage = {
		newLang:function(){
			$.validationEngineLanguage.allRules = {
				...
				"orderNum":{
					"regex": /^[0-9 ,]*$/,
					"alertText":"* 合同号输入不合法，查找多个用‘空格’、‘，’隔开"
				},
				"money":{
					"regex": /^\d{1,9}|\d{1,9}\.\d{1,3}$/,
					"alertText":"* 无效数值或数值过大"
				},
				"date":{
				...
				"chineseAndEnglish":{
			    	"regex":/^[a-zA-Z\u4e00-\u9fa5。，,.:“”{}（） \/]+$/i,
				    "alertText":"* 只能填写中文、英文、中文标点"
				},
				"chineseAndEnglishAndNumber":{
					"regex":/^[0-9a-zA-Z\u4e00-\u9fa5]+$/i,
					"alertText":"* 只能填写中文、英文、数字"
				},
				"onlyLetterNumber":{
					"regex":/^[0-9a-zA-Z]+$/,
				...
				"financingPercent":{
					"regex":/^0\.([1-9]|[0-9][1-9]|[0-9][0-9][1-9])$/,
					"alertText":"* 数据格式大于0小于1并且小数点后最多三位小数"
				},
				/**
				 * 自定义公用提示信息：
